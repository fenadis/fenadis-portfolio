...
import { hslToHex, getCSSColorValue } from '@/utils/colors';
    ...
    setTimeout(() => {
      simulation.current?.setConfig({
        colorPalette: [
          ...
          getCSSColorValue('--secondary'),
          getCSSColorValue('--accent'),
        ],
      });
    }, 0);
    return
  ...
  return (
    ...
    >
      <div className='fixed left-0 top-0 h-full w-full'>
        <div ref={containerRef} className='h-full w-full z-20' />
      </div>
      <main ref={mainRef} className='h-full w-full z-50'>
        {children}
      </main>
    </SimulationContext.Provider>
  );
